...
start('Shop Wallet & PayU Realtime Topup (Admin)');
?>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px; margin-bottom: 30px;">
    ...
    <div class="card">
        ...
        <form action="<?=url('/api/wallet-payu-init.php')?>" method="POST">
            <?php if (count($allShops) > 1): ?>
                <div class="form-group" style="margin-bottom: 14px;">
                    <label style="font-size: 0.82rem; color: var(--text-muted); font-weight: 600; text-transform: uppercase;">Select Shop to Credit *</label>
                    <select name="shop_id" onchange="location.href='?shop_id=' + this.value" style="width: 100%; padding: 10px 12px; background: rgba(15,23,42,0.6); border: 1px solid var(--border-color); border-radius: 8px; color: #fff; font-weight: 700;">
                        <?php foreach ($allShops as $sh): ?>
                            <option value="<?=$sh['id']?>" <?=$sh['id'] == $shopId ? 'selected' : ''?>>
                                <?=e($sh['name'])?> (Bal: <?=money($sh['wallet_balance'])?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
...
<div class="card">
    <h3 style="font-size: 1.1rem; font-weight: 800; color: #fff; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;">
        <i data-lucide="history" style="color: var(--primary);"></i> Overall Wallet Transaction History
    </h3>
