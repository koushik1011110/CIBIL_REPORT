...
$emiStmt = $p->query('
    SELECT e.*, f.application_no, f.customer_id, s.name as shop_name, c.name as customer_name, c.mobile as customer_mobile 
...
$q = $p->query('
    SELECT p.*, c.name as customer_name, f.application_no, s.name as shop_name, e.installment_no, e.due_date 
    FROM payments p 
...
start('All EMI Collections (Admin)');
?>
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px; margin-bottom: 30px;">
    ...
    <div class="card">
        ...
        <form action="<?=url('/api/manual-pay-emi.php')?>" method="POST">
            ...
            <div class="field" style="margin-bottom: 14px;">
                ...
                <select id="customerSelect" onchange="onCustomerChange(this.value)" required style="width: 100%; padding: 10px; font-size: 0.85rem;">
                    ...
                    <?php foreach($customersWithEmis as $cid => $cData): ?>
                        <option value="<?=$cid?>">
                            👤 <?=e($cData['name'])?> (<?=$cData['mobile']?>) [<?=e($cData['shop'])?>] — <?=count($cData['emis'])?> Pending Month(s)
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="field" style="margin-bottom: 16px;">
                ...
                <input type="text" name="remarks" id="remarksInput" value="Admin Manual Payment" style="width: 100%; padding: 10px;">
            </div>
    ...
    <div class="card">
        <h3 style="font-size: 1.1rem; font-weight: 800; color: #fff; margin-bottom: 12px;">Global Collections Info</h3>
        <p class="muted" style="margin-bottom: 16px;">First select the customer, then choose which pending EMI month to clear manually via cash payment.</p>
        ...
        <div style="background: rgba(15,23,42,0.6); padding: 16px; border-radius: 12px; border: 1px solid var(--border-color);">
            <div style="font-size: 0.8rem; color: var(--text-muted); text-transform: uppercase;">Total System Collected Amount</div>
...
<!-- COLLECTIONS TABLE -->
<div class="card">
    <h3 style="font-size: 1.1rem; font-weight: 800; color: #fff; margin-bottom: 16px;">System Collection History</h3>
...
function onCustomerChange(custId) {
    ...
    document.getElementById('remarksInput').value = 'Admin Manual Payment';
...
function onEmiMonthChange() {
    ...
    if (selectedOption && selectedOption.value) {
        ...
        document.getElementById('remarksInput').value = selectedOption.getAttribute('data-remark') || 'Admin Manual Payment';
    }
}
</script>
