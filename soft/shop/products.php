...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_product') {
    ...
    if ($delId > 0) {
        $delStmt = $p->prepare('DELETE FROM products WHERE id = ? AND (shop_id = ? OR ? = "superadmin")');
        $delStmt->execute([$delId, $sid, u()['role']]);
...
<div class="card" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
    <div>
        ...
        <p class="muted" style="margin-top: 4px;">Manage inventory products, Flipkart-style RAM/Storage variants, stock levels, and pricing</p>
    </div>
...
<div class="card" style="padding: 0; overflow-x: auto;">
    <table class="table" style="width: 100%; border-collapse: collapse; font-size: 0.85rem;">
        ...
        <tbody>
            <?php if(empty($rows)): ?>
                <tr><td colspan="8" style="text-align: center; padding: 20px;" class="muted">No products found. Click "Add New Product" to create one.</td></tr>
            <?php else: ?>
